...
class Gui(tk.Tk):
    ...
    def __init__(self, db: Database):
        ...
        self.entry1.bind('<FocusIn>',
                         lambda args: self.entry1.delete('0', 'end'))

        self.entry2 = tk.Entry(self, width=10)
        self.entry2.grid(row=0, column=1)
        self.entry2.insert(0, 'imie')
        self.entry2.bind('<FocusIn>',
                         lambda args: self.entry2.delete('0', 'end'))

        self.entry3 = tk.Entry(self, width=10)
        self.entry3.grid(row=0, column=2)
        self.entry3.insert(0, 'tytul')
        self.entry3.bind('<FocusIn>',
                         lambda args: self.entry3.delete('0', 'end'))

        self.entry4 = tk.Entry(self, width=10)
        self.entry4.grid(row=0, column=3)
        self.entry4.insert(0, 'data zwrotu')
        self.entry4.bind('<FocusIn>',
                         lambda args: self.entry4.delete('0', 'end'))

        self.combobox = ttk.Combobox(self, values=db._get_all_user_id(),
                                     width=5)
